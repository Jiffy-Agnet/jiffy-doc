...
```
                 ┌────────────────────┐
 @jiffy mention  │   Edge Component   │   filtered payload    ┌───────────────────────┐
 on an Issue ───▶│ (GitHub/GitLab/    │──────────────────────────▶│        Gateway        │
   or comment    │  Gitea, per-repo)  │  X-JIFFY-TOKEN header  │ Django + Celery+Redis │
                 └────────────────────┘                        └───────────┬─────────┘
                                                                            │ queues task
                                                                            ▼
                 ┌────────────────────┐                        ┌─────────────────────┐
                 │   Issue comment    │   summary + PR link    │        Sandbox        │
                 │ (report + PR link) │◀───────────────────────│  isolated Docker      │
                 └────────────────────┘                        │  container running    │
                                                                │  the OpenCode agent   │
                                                                └───────────┬─────────┘
                                                                            │ commit, push,
...
The payload is sent over HTTPS to the Gateway's provider-specific ingestion endpoint, authenticated with a shared secret in the `X-JIFFY-TOKEN` header — one secret per provider, so a leaked GitHub token can't be used to submit fake GitLab tasks. The Gateway (Django, with Celery and Redis for the task queue, SQLite by default) validates the payload, records the task, and queues it for the next free worker.
...
A Celery worker picks up the task and starts an ephemeral Docker container against an isolated copy of the target repository. The sandbox is a Debian bookworm image preloaded with the language toolchains a task might need (`nvm`, `uv`, `gvm`) and each provider's CLI (`gh`, `glab`, `tea`) for cloning, committing, and opening the PR. Inside it, the **OpenCode** coding agent reads the Issue thread, makes the change, and commits it.

The worker never talks to the host's Docker daemon directly — a Docker Socket Proxy sits in between and only allows the specific operations the worker needs (create/start/stop a container, stream its logs), so a compromised task can't reach anything else running on the host.
</Step>
...
| Sandbox | A Docker container spawned by the Gateway's worker | Per task — created, runs, torn down |
